...
The Ops Center is a web-based management tool that enables you to manage and monitor Anypoint Platform Private Cloud and perform the following types of tasks:
...
Anypoint Platform Private Cloud enables you to forward server-level log data. Log forwarding is designed to forward your logs to a local 3rd party log solution. Forwarding log data to a cloud-based service, like Splunk Cloud, is not supported.
...
Anypoint Platform uses `rsyslog` to handle logging. To forward these logs to remote hosts, perform the following steps:
...
. Configure your log forward as shown in the following example:
