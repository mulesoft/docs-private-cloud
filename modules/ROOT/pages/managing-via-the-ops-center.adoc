...
Anypoint Platform Private Cloud Edition enables you to forward server-level log data. Log forwarding is designed to forward your logs to a local 3rd party log solution that accepts syslog format log data. Anypoint Platform uses `rsyslog` to handle logging. The built-in `rsyslog` client service enables you to send log data to an rsyslog server over TCP or UDP. Forwarding log data to a cloud-based service, like Splunk Cloud, is not supported.

 To forward logs to remote hosts, perform the following steps:
...
. Click *Create* or *New Connector* if you already have a log forwarder configured.

. Configure your log forwarding by specifying values for `name`, `address` and `description`, using the following code as a guideline. 
+
[%header,cols="2*a"]
|===
|Key | Description
...
|`protocol` | Specifies the protocol to send the data to. Supported protocols are TCP or UDP. Defaults to `tcp`. Specify `udp` for the UDP protocol.
|===
+ 
----
kind: logforwarder
...
Your logs will now be forwarded to your external logging service. Any other steps needed here???

[NOTE]
* Log information is stored for up to ??? days. Logs older than xx-days are permanently removed (overwritten)???
// What is true here???
