...
Anypoint Platform PCE uses Docker and Kubernetes for built-in high availability and scalability. Understanding these tools is essential for management and troubleshooting.

Anypoint Platform PCE version 4.0 runs on enterprise-ready Kubernetes RedHat OpenShift, Amazon EKS Anywhere (EKS-A), and Rancher Kubernetes Engine (RKE2). Additionally, it supports Java 17, in addition to Java 8 and 11.

For information about a previous version of Anypoint Platform PCE, select the version in the documentation sidebar.

== Before You Begin

=== Upgrade to Anypoint Platform PCE version 4.0
If you are upgrading from version 3.2.3 of Anypoint Platform PCE to version 4.0, see xref:upgrade.adoc[Upgrading PCE 3.2.3 to 4.0].

[NOTE]
If you're using a version of Anypoint Platform older than 3.2.3, upgrade to version 3.2.3 first. Then proceed to upgrade to version 4.0.
...
== Add-On Features Supported by Anypoint Platform PCE 4.0

The following table provides add-on features supported Anypoint Platform PCE 4.0:

[%header%autowidth.spread]
|===
| Component | Description
| Anypoint Monitoring | Provides visibility into integrations, Mule flows, and components in your application network.
|===
