...
There are differences differences between Anypoint Private Cloud and the cloud version of Anypoint Platform you need to consider when uploading custom policies and publishing assets to Exchange using the Maven Client.
...
----
+
In the examples below, ${ANYPOINT_DNS} is set to your DNS for Anypoint Platform Private Cloud. The `username` and `password` must correspond with your installation.
...
----
+
In this example, $ANYPOINT_DNS is set to your DNS for Anypoint Platform Private Cloud. Replace the location and password for your JVM’s trustore for your environment.
