...
You can limit concurrent login to Anypoint Platform Private Cloud Edition (Anypoint Platform PCE) so that a user can sign in from only one browser or device at a time. This applies to users in your organization who log in to the platform (for example, to use Runtime Manager, API Manager, or Access Management). When you limit concurrent login, a new sign-in from another browser or device signs the user out of the previous session.
