...
# Platform configuration for the shared ReadWriteMany (RWX) filesystem.
# Choose exactly one of the two storage modes below. NFS mode has two configuration
# variants (Option A or Option B). StorageClass mode is Option C. The modes are
# mutually exclusive.
platformConfiguration:
  platformDNS: ""
  # ---- NFS mode (Option A and Option B) ----
  # Requires a pre-installed CSI driver in the Kubernetes cluster.
  fileSystemCsiDriverName: "" # CSI driver name (e.g., nfs.csi.k8s.io, efs.csi.aws.com)
  # Optional NFS mount option overrides. Comma-separated string. Only used in NFS mode.
  fileSystemBareMountOptionsOverride: ""
  fileSystemWcMountOptionsOverride: ""
  fileSystem01MountOptionsOverride: ""
  # Option A - Generic NFS CSI driver: provide DNS and path.
  # fileSystemDNS: ""   # NFS server hostname or IP
  # fileSystemPath: ""  # NFS export path (e.g., /exports/pce)
  # Option B - CSI driver with individual volume handles (all 4 required, e.g. EFS CSI Driver
  # with 4 access points that point to the same directory in the EFS):
  ...
  # ---- StorageClass mode (Option C, Anypoint Platform PCE 4.3.0 and later) ----
  # Mutually exclusive with NFS mode. The StorageClass must support ReadWriteMany.
  # Leave fileSystemCsiDriverName and all NFS-mode fields above empty when using this option.
  # For backup retention, use a StorageClass with reclaimPolicy: Retain so deleting the PVC
  # does not destroy historical backup tarballs.
