...
. Configure DNS again to trigger the creation of certificates for the new Runtime Manager services. If the DNS setting already has the desired value, copy and paste it again in the field to trigger a form change so that you are able to click Save. Refer to the xref:access-management-dns.adoc[Configure DNS on Anypoint Private Cloud] section.
+
WARNING: If you cannot access Anypoint Access Management or the platform, follow the https://pce-runbook.kbuild.msap.io/content/2.1.x/installation/setting-user-dns-nfs.html#wrong-setup-of-the-platform-dns[Wrong setup of the platform DNS] runbook entry to configure DNS via the command line.
