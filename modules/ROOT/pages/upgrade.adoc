...
Refer to the xref:release-notes::pce/anypoint-private-cloud-2.0.3-release-notes#upgrade-paths[Upgrade Paths) section in the Anypoint Platform PCE release notes for information about upgrading Anypoint Platform PCE to the most recent version.
...
If you are using AWS, obtain the keys used to run the AWS provisioner. These are required for access EC2, VPC, and S3.
...
* The upgrade process does not include your applications and data. You must upgrade your applications to work with the new version of Anypoint Platform PCE.
====
...
+
[WARNING]
If you are upgrading from 1.7.x -> 2.0.x, ensure that you configure your NSF servers described in the following step before accessing Anypoint Platform PCE. In this case, failure to configure NFS before accessing the platform can result in data loss.

. If you are upgrading from 1.7.x -> 2.0.x,, configure the required NFS server.
+
...
. Unless you are updating Anypoint Platform PCE from 2.0.x to 2.1.0, configure the following items:
