...
. Verify there is adequate space available in the path where you plan to download the new installer file.
...
. When the upgrade finishes, verify the migration was successful by checking that all pods are running:
+
----
$ sudo gravity enter
...
. Finally, make sure the following items are still correctly configured after the update:
