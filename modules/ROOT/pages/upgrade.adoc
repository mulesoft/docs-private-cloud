...
Refer to the xref:release-notes::pce/anypoint-private-cloud-2.1.0-release-notes.adoc#upgrade-paths[Upgrade Paths] section in the Anypoint Platform PCE release notes for additional information about upgrading Anypoint Platform PCE to the most recent version.
...
. Navigate to the `anypoint-2.1.0` directory and run the upload script:
+
----
cd anypoint-2.1.0
sudo ./upgrade
...
. Make sure the following items are configured correctly:
