...
To upgrade Anypoint Platform Private Cloud Edition version 3.2.3 to version 4.0:
...
. In your Anypoint Platform PCE v3.2.3 cluster, scale down `nginx-wss-service` to 0 replicas. After 5 minutes, scale it up to 3 replicas.

>>>>>>> 604743e1675647a0b82091624488432688dac806
[[create-backup]]
== Create a Backup
...
data:
 ...
 ignore-releases: "core-paas-namespaces stolon stolon-amv pce-seaweedfs-app pce-cluster-ssl-app dias-prov-k8s-am-influxdb-comp dias-prov-k8s-am-ingestor-comp"
  ...
  seaweedfs_master_pod_count: "3"
  seaweedfs_filer_pod_count: "2"
  seaweedfs_volume_pod_count: "3"
  restore_s3_max_attempts: "3"
  restore_s3_sleep_time_seconds: "30"
  restore_in_parallel: "false"
  restore_timeout_seconds: "10800"
----
+
[start=2]
...
+
[start=5]
. Extract the contents of the `tar` files to merge the projects and workingDirectories folders. Additionally, ensure that the gravity `tar` file is also extracted during this process. 
