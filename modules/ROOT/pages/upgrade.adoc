...
. Verify that there is adequate disk space available for the upgrade, which is at least half the amount specified in xref:prereq-hardware.adoc[Hardware Prerequisites]. For example, for `/var/lib/gravity`, the recommended amount of disk space is 100Gb, so ensure that you have 50Gb of available space. Run the following command to check the amount of available space:
+
----
sudo df -h /var/lib/gravity/
Filesystem      Size  Used Avail Use% Mounted on
/dev/nvme0n1p2  200G   66G  135G  33% /
----
+
If you do not have enough available space, run the garbage collection command, pointing to the `gravity` binary from the new installer:
+
----
sudo ./gravity gc
----
+ 
If running the garbage collection does not free up enough space, you must extend the disks before performing the upgrade.
