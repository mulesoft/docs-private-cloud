...
. Use the following spec replacing the values with your SMTP config and create a file named `smtp-config.yaml` inside gravity:
...
. Add a default router to Alertmanager config by running this one-liner inside gravity:
...
. Configure the FROM email address by replacing <SMTP_FROM> in the next one-line and running it inside gravity:
...
. Restart alertmanager pods by running the following:

    kubectl delete pod -n monitoring -l app=alertmanager

. Test Alertmanager by running the following inside gravity:
