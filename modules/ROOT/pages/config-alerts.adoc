...
|===
| Name | Description | Threshold | Configmap
...
| Etcd Latency | Measures the latency between nodes. Latency can affect `etcd` performance | Warning 521 seconds, Critical 1024 seconds | kapacitor-alerts 
...
| Node Uptime | Checks if nodes are online | When a node is down for more than 5 minutes | kapacitor-alerts 
| Etcd Health | Check the status of `etcd` | When `etcd` is down or not healthy | kapacitor-alerts 
| Docker Status	| Verifies the docker service is running on each node | When docker service is not running on a node | kapacitor-alerts 
| Backup Failing | Check the result of a backup operation | When a backup operation fails | backup-status-alert
| Stolon Replication Lag | Check `Postgres` replicas are in synch | When a replica is lagging by 1800 or more seconds | kapacitor-alerts 
| Cassandra Load | Verifies Cassandra nodes are in sync | Warning 10% difference, Critical 20% difference | cassandra-load
|===
