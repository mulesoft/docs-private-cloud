...
| System | Uptime | Triggers a warning when a node's uptime is less than 5 minutes
...
| Etcd | Etcd instance health | Triggers an error when an etcd master is down longer than 5 minutes
| Etcd | Etcd latency check | Triggers a warning when follower <-> leader latency exceeds 500ms, triggers an error when it exceeds 1 second over a period of 1 minute
