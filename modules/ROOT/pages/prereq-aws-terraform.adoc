...
+	https://anypoint-anywhere.s3.amazonaws.com/pcp/private-cloud-provisioner-2.1.0.tar.gz?AWSAccessKeyId=AKIAI6LBJNQFR2V3CQWQ&Signature=fN9D6u%2FEmofbD1UGgxiqNXsP9mE%3D&Expires=1743046273
...
----
docker run --rm -v $(pwd)/pre-user-data:/usr/local/bin/provisioner/terraform/external/pre-user-data -v $(pwd)/post-user-data:/usr/local/bin/provisioner/terraform/external/post-user-data --env-file pce.env artifacts.msap.io/mulesoft/core-paas-private-cloud-provisioner:v2.1.0 cluster-provision
----
...
----
docker run --rm --env-file pce.env artifacts.msap.io/mulesoft/core-paas-private-cloud-provisioner:v2.1.0 cluster-deprovision
----
