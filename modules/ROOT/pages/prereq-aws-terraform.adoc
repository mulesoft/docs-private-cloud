...
| AWS_SESSION_TOKEN | Specifies AWS temporary security credentials.
...
. Record the image ID after running the `docker load` command. Run the following command to retrieve the image ID, which is required for the next step:
+
```
gravity images
What do we need to say if there are multiple images? How does the user know which one to use?
```
