...
You can destroy all the resources that were created using the provisioner by running `cluster-deprovision`. Make sure the `TELEKUBE_CLUSTER_NAME` env variable in your environment file has the right value for the target cluster you want to destroy:
