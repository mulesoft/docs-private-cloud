...
This topic describes how to create the resources required to install Anypoint Platform Private Cloud on Amazon Web Services (AWS). Anypoint Private Cloud supports 3-node and 6-node configurations in a production environment on AWS.

These steps are not needed if you are starting from bare metal servers.

[IMPORTANT]
Your infrastructure team, or someone with AWS administrator level knowledge, must perform the following steps. If needed, contact your MuleSoft representative for assistance.

== AWS Account Permissions and Resources

To install Anypoint Platform Private Cloud on AWS:
...
. Download the Private Cloud Provisioner (PCP) Docker image from https://anypoint-anywhere.s3.amazonaws.com/pcp/pcp-1.0.tar.gz?AWSAccessKeyId=AKIAI6LBJNQFR2V3CQWQ&Expires=1712193983&Signature=noiRGBIBJtwsMLQxTux%2FiM%2F5SP0%3D.
...
|===
| Name | Description
| TF_VAR_ami_name | Specifies the AMI name to be used for the instances. Use the AMI name and NOT the AMI ID. If you do not set this variable, the provisioner uses the following AMI by default: `RHEL-7.4_HVM_GA-20170808-x86_64-2-Hourly2-GP2`
...
If you are installing Anypoint Private Cloud using the GUI-based installer, you must enable this port before running the installer. You must open this port to the world in the cluster's security group before running the installer using AWS Web Console.
...
The AWS account might have some limits set on the amount of the resources you can create. Delete some unused resources or request a limit increase on AWS.
