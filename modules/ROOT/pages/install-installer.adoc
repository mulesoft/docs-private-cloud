...
// Where do I put the gzip archive??? Where does the license file go? The license file is only needed on the installer node.
...
// Does PCE use the same license as Mule Runtime? Assumption is yes. Is the license file the same as the license key?
+
