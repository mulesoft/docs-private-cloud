...
+
[WARNING]
If you are running the above command from a ssh session ensure the session does not time out during installation or the installer will fail.
Using `nohup` and `&` at the end, e.g. `sudo nohup bash &` will make the process to start in the background and will keep running if your ssh session expires.
