...
WARNING: As requests to external domains from the Exchange Public Portals go through the API Console Proxy, they will fail as no Anypoint user is involved in the call.


*Non-Strict Authentication*: API Console Proxy continues to be unauthenticated, but also allows unauthenticated requests. The incoming request must provide either the user's token or a reference to the Organization ID that the request belongs to. The Organization ID must be sent on the `ms-organization-id` header.

To change the authentication method from strict to non-strict, follow these steps:

. Log in to Ops Center and select *Kubernetes* on the left side-bar.
. Select the `api-console-proxy` namespace from the drop-down menu on the top, and then select *Configmaps*. Find the `service-env` configmap and then click on *Edit Config Map*.
