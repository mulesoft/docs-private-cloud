...
Before you begin, make sure that your organization’s operations, networking, and security teams review the listed infrastructure prerequisites. These include the K8s cluster, hardware, software, networking, shared filesystem, container registry, installer node, and storage requirements.
...
Installing Anypoint Platform PCE requires specific versions of operating systems, shared filesystem components, and other software. Most issues experienced with Anypoint Platform PCE occur because the required environment was not set up prior to installing or upgrading.
====
...
=== Sample Worker Node Configurations

Anypoint Platform PCE offers flexibility in how you configure your Kubernetes worker nodes. You can use different numbers of worker nodes (for example, 4, 5, 8, or 10 nodes) with varying resource allocations per node. The key requirement is that your cluster's total resources meet or exceed the minimum requirements of 32 vCPUs and 128 GiB memory.

This table shows sample worker node configurations that MuleSoft's performance teams tested and validated:

[%header%autowidth.spread]

|===
|Number of Worker Nodes |Node Composition |Total Resources
|4 |8 vCPU and 32 GiB memory |32 vCPU and 128 GiB memory
|10 |4 vCPU and 16 GiB memory |40 vCPU and 160 GiB memory
|===

[NOTE]
These configurations are reference examples to help you understand how Anypoint Platform PCE can work with different worker node configurations. They are not strict requirements. You can customize your cluster configuration based on your infrastructure and operational needs, as long as the total resource requirements are met.
...
+
AWS EKS is only tested with default networking add-ons.
* https://docs.rke2.io/install/requirements#hardware[Rancher Kubernetes Engine (RKE2) - Hardware]
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/installation_overview/ocp-installation-overview[Red Hat OpenShift]
* https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vsphere-supervisor-services-and-standalone-components/latest/managing-vsphere-kubernetes-service/running-tkg-service-clusters/tkg-service-components.html[vSphere Kubernetes Service (VKS)]
...
Use these ports to communicate with Anypoint Platform PCE:
...
Anypoint Platform PCE nodes need to hit the platform API address on the load balancer via HTTPS.
...
* https://docs.redhat.com/en/documentation/openshift_container_platform/4.16/html/networking/understanding-networking[Red Hat OpenShift - Networking]


== Shared Filesystem Requirements

Anypoint Platform PCE requires a `ReadWriteMany` (RWX) shared filesystem. Starting with Anypoint Platform PCE 4.3.0, you can provide this filesystem using either of the following storage modes:

* *NFS mode* — Bind the platform to an existing NFS server through a pre-installed Container Storage Interface (CSI) driver in the Kubernetes cluster. This is the only supported mode in versions before 4.3.0.
* *StorageClass mode* — Provide a single RWX-capable Kubernetes `StorageClass` (for example, backed by CephFS) and let Anypoint Platform PCE dynamically provision the shared volumes. This mode removes the requirement to operate an external NFS server.

In StorageClass mode, Anypoint Platform PCE requests these persistent volumes:

* Design Center: 2 PVCs, 300 GiB each
* Backup and restore: 1 PVC, 1 TiB

The two modes are mutually exclusive. You cannot change the storage mode on an existing cluster. The installer rejects an in-place upgrade from NFS mode to StorageClass mode so that data on the previous volumes is not orphaned. Switching the NFS CSI driver during an upgrade does not require additional storage changes.

For detailed configuration instructions, including CSI driver setup, EFS-specific volume handle setup, NFS mount option overrides, and `StorageClass` requirements, see xref:verify-nfs.adoc[].
...
Before executing the `pcectl` binary for Anypoint Platform PCE 4.2, make sure that you follow these requirements:
