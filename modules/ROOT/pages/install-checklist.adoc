...
Before you install Anypoint Platform Private Cloud, your organization’s operations, networking, and security teams must review the listed infrastructure prerequisites, including Amazon Web Services (if applicable), hardware, software, network, and NFS, and verify successful completion of all prerequisites listed in this topic. If needed, contact your Customer Success Manager for assistance.
...
In addition, to install Anypoint Platform Private Cloud, you must possess knowledge in the following areas:

* Linux system commands. 
* Working in Linux with root access. You must have root access (permission to run the `sudo` command) on all servers in your environment and run the installer or upgrade tool as root to access specific privileged ports.
* Remote connection mechanisms, such as SSL and SSH.

[WARNING]
==== 
Installing Anypoint Platform Private Cloud requires specific versions of operating systems, NFS, and other software. Most problems experienced with Anypoint Platform Private Cloud occur because the required environment was not set up prior to installating or upgrading. For example, the following situations cause known problems:
...
|===
|Task Description
| xref:supported-cluster-config.adoc[Review Supported Network Topologies for Anypoint Private Cloud]
...
|===
|Task Description
| xref:prereq-other#verify-space-in-home-directory[Installation Package Requirements]
...
|===
|Task Description
| xref:prereq-gravity-check[Verify Environment]
|===
...
|===
|Task Description
| xref:prereq-other[Gather Environment Information]
|===
