...
You can configure your load balancer to use any method for distributing client requests. Use a a round robin strategy for better results in most contexts. Make sure the load balancer is accessible through an IP address that is available to all machines in your network.

Make sure your load balancer routes the following TCP ports:
...
[NOTE]
Do not configure SSL certificates in your load balancer. The platform handles the TLS termination with the certificates configured using Access Management. See xref:config-workflow.adoc[Configure Anypoint Platform PCE].
