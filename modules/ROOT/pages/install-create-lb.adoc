...
[NOTE]
If using the xref:prereq-aws-terraform.adoc[AWS Provisioner for Anypoint Private Cloud Edition], an Amazon ELB is already created with the required configuration and you can skip this section.
...
[NOTE]
You must not configure SSL credentials in your load balancer. TLS termination is done by the platform with the certificates configured using Access Management. See xref:install-add-tasks.adoc[To Perform Additional Installation Tasks]

== Example: Implementing a Load Balancer using Nginx 
