...
To ensure the performance and stability of Anypoint Platform Private Cloud Edition, every node in your Anypoint Platform Private Cloud Edition environment must meet the software prerequisites described in this topic:
...
== Operating System Preqrequisites
...
You can install LVM using Yum. Yum is an open-source command-line package-management utility for Linux operating systems using the RPM Package Manager. Using Yum, you can install LVM using the following command:
...
* Software to Verify NFS Server Ports are Open

** Netcat is recommended, but you can use your tool of choice to verify the correct ports are open on the NFS server. If you don’t have `netcat`, you can install it from  http://netcat.sourceforge.net/download.php or follow the instructions at https://linoxide.com/linux-how-to/install-use-netcat-command-linux/.

* Software to Verify Kubectl Command is Available

** Gravity is recommended, but you can use your tool of choice to ensure the `kubectl` command is available on the node where you are performing the installation or migration. To verify that `kubectl` is installed using Gravity, run the following command:
+
----
$ sudo gravity enter
$ kubectl
