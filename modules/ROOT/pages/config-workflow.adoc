...
[NOTE]
At the time of this writing, some Docker containers must run as root. Current work in progress removes the need for these containers to run as root. Containers running as root
are not exposed outside the cluster. Containers that are exposed outside the cluster run as non root.
