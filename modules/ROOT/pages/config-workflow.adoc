...
[NOTE]
Some Docker containers must run as root for port binding and system and kernel configuration. Containers running as root
are not exposed outside the cluster. All containers that are exposed outside the cluster run as non root.
