...
[NOTE]
At the time of this writing, some Docker containers must run as root. Current work in progress removes the need for 
these containers to run as root. Processes running as root do not expose functionality outside the cluster. 
Processes that serve external traffic run as non root to maximize security.
