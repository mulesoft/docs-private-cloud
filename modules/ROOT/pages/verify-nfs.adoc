...
On each node, use the netcat tool to verify that the required ports are open. The netcat tool is a computer networking utility used to read and write to network connections using TCP or UDP. 
...
----
// My output didn't look like this???

== Mount the NFS Server
// Is this done on all nodes???
...
// So what do I do to get the required permissions???
