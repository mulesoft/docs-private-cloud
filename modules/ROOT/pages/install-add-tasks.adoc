...
The developers of your organization that create Mule apps through the xref:6.x@studio::index.adoc[Anypoint Studio] IDE should set up their editors so that these interface with your locally installed Anypoint Platform (instead of the default cloud Anypoint Platform). See xref:6.x@studio::setting-up-your-development-environment.adoc#enable-on-premises-setup[Enable on-premises Setup].
