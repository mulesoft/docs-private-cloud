...
After verifying that the cluster is healthy, select a node on which to perform the following steps:
...
```
The previous command outputs a `systemd` unit name, which is used as the `$unit_name` value in the next step.
...
```
. After the `status` subcommand indicates the gravity unit is stopped, install the needed updates and reboot the server.
...
```
The previous command should return zero units, which means that your system boot phase was successful and all units started. If any units failed to start, fix all issues before proceeding.
...
```
The target node is set to again schedule pods. This can take a few minutes to complete.
