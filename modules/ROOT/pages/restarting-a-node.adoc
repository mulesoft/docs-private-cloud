...
[NOTE]
All clusters should have backup setup and tested as the very first thing after installation.
The installation phase should not be considered done until backups are completely setup and tested.
[NOTE]
To preserve your data in case of system failure, ensure that your system is backed up right before attempting node maintenance.
[NOTE]
The steps below must be performed on Anypoint Platform PCE on a single node at a time.  If maintenance is required on more than one node, the entire process must be completed on a single node before the next node is addressed.
