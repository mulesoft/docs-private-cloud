...
To preserve your data in case of system failure or problems with other operations, such as patching or performing upgrades, ensure that your system is automatically backed up. Regular backups are extremely important for disaster recovery procedures. Configure your backup to run at least once per day; hourly is recommended. Store your backup archive on external storage outside the Anypoint cluster.
...
====
+
Wait until the operation completes. This operation takes a few minutes. 

. Manually restore the following information: 
+
* Runtimes
