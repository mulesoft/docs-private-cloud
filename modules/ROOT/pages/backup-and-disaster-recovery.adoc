...
* `--timeout`
+
Specifies the deadline for the backup job (e.g.: 30s, 5m, etc). Default is `80m`. If you have a very large number of applications and files use `160m` as the timeout at least.
* `--log-file`
+
Outputs logs to a file.
====

. Wait for the operation to complete (~40-60 minutes).

. Manually restore the NFS files if you are not reusing the NFS and you are using a different one.
