...
[IMPORTANT]
====
Backup and restore for Anypoint Monitoring and Anypoint Visualizer are performed outside the normal Gravity backup and restore mechanism, and require a separate procedure.
====
...
This command creates an archive of the current system state in: `/var/lib/gravity/planet/share/backup.tar.gz`. You can pass the `--follow` parameter to the `gravity` backup command to outputs the backup job logs to stdout:
+
----
...
. Move the compressed backup archive file to a folder on any master node in the environment to be restored. You can transfer this file securely by using the following command:
+
----
...
----
+
You can pass `--follow` to output the restore job logs to stdout.
...
The backup and restore script must be run on a `master` node and must have `sudo` access because it uses `kubectl`. The script validates the restore disk size, databases, measurements, retention policies, and series cardinality for all measurements in each database.

=== Procedure

The InfluxDB backup and restore script is in the PCE environment: `/var/lib/gravity/site/packages/unpacked/gravitational.io/anypoint/3.0.0*/resources/kubernetes/amv-backup-restore/amv-backup-restore.sh`
...
----
 ...
 -f : (Required) name of the tar file.
        Default for backup : dias-prov-k8s-am-influxdb-comp_YYYYMMDD.  [string].
