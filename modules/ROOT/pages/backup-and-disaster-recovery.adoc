...
This command creates an archive of the current system state in: `/var/lib/gravity/planet/share/backup.tar.gz`. You can pass the following optional parameters:

----
--follow: output backup job logs to stdout
--timeout: deadline for the backup job (e.g.: 30s, 5m, etc). Default is 20m.
