...
To restore a system from your backup use the same existing cluster or create a new cluster.

[WARNING]
Some settings like NFS, Certificate and DNS are not backed up and they should be setup on the environment where there restore is going to be performed *before* running the restore.
...
* `--timeout`
+
Specifies the deadline for the backup job (e.g.: 30s, 5m, etc). Default is 80m.
* `--log-file`
+
Outputs logs to a file.
====
