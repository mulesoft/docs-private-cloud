...
Monitoring Center can be enabled as an additional add-on. Monitoring Center runs on 3 nodes separate from the nodes that run the Anypoint Platform Core. These nodes host the stateful and stateless services that compose Monitoring Center.
