...
These tests are run using large files:
...
. Perform writes (five files of 128MB each) on the NFS server by running the following command:
...
==== Validate Reads from the NFS Server

The important parameter in the test output is the `real` parameter, which should be around 10 seconds.
...
To start testing verify the `/mnt/home` directory from the previous test is created, then follow these steps:
