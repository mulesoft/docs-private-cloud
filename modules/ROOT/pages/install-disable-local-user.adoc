...
. Before deleting the local user, perform the following steps:
...
+
image::access-management-user-id.png["Example of User ID in the URL"]

. Configure your installer machine with `kubeconfig.yaml to access the Anypoint Platform PCE K8s cluster.
.. Set the environment variable `KUBECONFIG=/path/to/kubeconfig.yaml`.
.. Check access to kubernetes cluster with `kubectl get nodes`.
...
.. From Anypoint Platform, select *Access Management*.
.. Select *Users*.
.. Select `username` in the same row as `SystemAdmin`.
.. Select *Disable*.
.. Select *Delete*.
