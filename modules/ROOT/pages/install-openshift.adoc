...
The following snippet provides an example installation manifest for deploying Anypoint Platform PCE on an OpenShift cluster. The example configures the platform shared filesystem in StorageClass mode (Anypoint Platform PCE 4.3.0 and later) using an OpenShift Data Foundation CephFS `StorageClass`. For the equivalent NFS-mode configuration and the full set of shared-filesystem options, see xref:verify-nfs.adoc[].
...
postInstallConfiguration:
  ...
  fileSystemStorageClass: ocs-storagecluster-cephfs
  firstUserAccount:
