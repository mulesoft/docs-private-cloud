...
The error message `mount failed: exit status 32. Mounting command ... not permitted` may occur when trying to mount with the `noresvport` option since the ports may be limited. If this happens, remove the `noresvport` option and add the `insecure` flag to the mounting command.
