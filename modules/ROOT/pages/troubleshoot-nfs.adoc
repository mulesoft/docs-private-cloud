...
[NOTE]
This page applies only when installing Anypoint Platform PCE in NFS mode. If you are using StorageClass mode (Anypoint Platform PCE 4.3.0 and later), troubleshoot storage issues through your `StorageClass` provider and CSI driver instead. See xref:verify-nfs.adoc#storage-class-mode[StorageClass Mode].
