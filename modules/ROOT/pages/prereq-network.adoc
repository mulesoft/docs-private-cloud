...
receive packages with a source and destination that is different from server’s internal IP.
...
To configure an Exernal Identity Provider for users single sign on (LDAP, OpenID, SAML) or for external client management as OpenID Connect, the cluster must be able to access the External Identity Provider endpoint. 
