...
== RHEL 8.4+ on AWS limitations
In some operating systems like RHEL 8.4, NetworkManager includes two extra services called `nm-cloud-setup.service` and `nm-cloud-setup.timer`. These services add a routing table that interfere with the Kubernetes CNI plugin's configuration. If those services exist, they should be disabled and the node must be rebooted before proceeding with the installation.

Anypoint Platform PCE nodes:: Nodes that are members of the cluster.
installer node:: Node on which the installation was started.
...
|TCP | 10255 | Kubernetes Kubelet (read-only) | Anypoint Platform PCE nodes | Anypoint Platform PCE nodes
