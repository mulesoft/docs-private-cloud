...
// https://docs.mulesoft.com/private-cloud/2.0/prereq-verify-device - ADD page alias???????????
...
//Have a check for each device - get from Fed???????????
...
All devices must be formatted either as *xfs* or *ext4*.
...
[NOTE]
A minimum of 100Gb is required for the Device Mapper directory. Devices with 50Gb or less will experience degraded system performance or might not work at all.
...
// Is Device Mapper specific to Docker? NO . THIS IS A DUP FROM UNINSTALL INSTRUCITONS. POSSIBLY MOVE TO MANUAL RESET SECTION. What does ‘device’ mean here? Is it an unmounted thing or one of 4 topics described? How do you reset a partition? What does ‘reset’ mean? Says remove. Where is this run from???
