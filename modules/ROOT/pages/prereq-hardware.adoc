...
|HDD 4 xref:prereq-hardware#docker-device[Docker Device] | 100 GB |3000|Not mounted | Used by Docker’s Overlay storage driver. A minimum of 100Gb is required for the Overlay directory. Devices with 50Gb or less will experience degraded system performance or might not work at all.
...
[NOTE]
Make sure this device is mounted AFTER mounting the System State device under `/var/lib/gravity/planet`. If you are using system.d units to mount the volumes you can declare mount dependencies using `After=<device-name>.<mount>`

== Docker Device

Unless specified, Docker configuration defaults to the use of Overlay driver, which is recommended for production.

Format this device either as `xfs` or `ext4` and mounted as `/var/lib/gravity/planet/docker`.

The following shell snippet is provided as an example for mounting the Docker device using systemd mount files, and cannot be used as-is in production. The end result of this step mounts the Docker device to the path `/var/lib/gravity/planet/docker`. You can also mount the Docker device using `/etc/fstab` or some other method. Consult with your system administrator to determine the best way to meet this requirement. If you use the following example to guide this process, make sure to specify the correct device name in two places.
