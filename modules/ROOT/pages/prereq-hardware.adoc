...
Make sure that you have at least 10GB in your home directory to download and unzip the installer file.
...
Anypoint Platform Private Cloud has strict minimum size and performance requirements for storage devices used by the platform. All disk drives must meet the following criteria:
...
==== Manually Reset Docker Device and Partitions
...
Refer to xref:install-create-lb.adoc[Set up a Load Balancer] for detailed information.
Anypoint Platform Private Cloud must run in a production environment with multiple servers. To distribute traffic among servers and to restrict access to specific ports, you must install and configure a load balancer. 
