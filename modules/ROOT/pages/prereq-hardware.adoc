...
|===
| Component |Size|Minimum IOPS|Mount Point|Description
|HDD 2 xref:prereq-hardware.adoc#system-state-directory-device[System State Directory Device] | 250 GB |1500|/var/lib/gravity | Stores system configuration and metadata, including databases and packages. Package sizes can be large, so it is important to estimate the minimum size requirements and allocate enough space as a dedicated device before installation.
|HDD 3 xref:prereq-hardware.adoc#docker-device[Docker Device] | 100 GB |3000|/var/lib/gravity/planet/docker | Used by Docker's overlay storage driver. A minimum of 100GB is required for the overlay directory. Devices with 50GB or less will experience degraded system performance or might not work at all.
|HDD 4 xref:prereq-hardware.adoc#dias-amv-device[Cassandra AMV Device]| 500 GB |1500|/var/lib/data/dias-meld & /var/lib/data/dias-cassandra | Stores Cassandra database data for Monitoring Center Add-On. The amount of space required should be at least 250 GB, but can vary depending on your specific use case. It is important to estimate the minimum size requirements and allocate enough space as a dedicated device ahead of time.
|HDD 5 xref:prereq-hardware.adoc#stolon-amv-device[Stolon AMV Device]| 250 GB |1500|/var/lib/data/stolon-amv | Stores Postgres database data for Monitorng Center Add-On. The amount of space required should be at least 250 GB, but can vary depending on your specific use case. It is important to estimate the minimum size requirements and allocate enough space as a dedicated device ahead of time. 
|HDD 7 xref:prereq-hardware.adoc#influxdb-amv-device[InfluxDB AMV Device]| 4 TB |1500|/var/lib/data/influxdb | Stores InfluxDB data for Monitoring Center Add-On. The amount of space required should be at least 250 GB, but can vary depending on your specific use case. It is important to estimate the minimum size requirements and allocate enough space as a dedicated device ahead of time.
|HDD 7 xref:prereq-hardware.adoc#logstash-amv-device[logstash AMV Device]| 500 GB |1500|/var/lib/data/logstash | Stores Logstash data for Monitoring Center Add-On. The amount of space required should be at least 250 GB, but can vary depending on your specific use case. It is important to estimate the minimum size requirements and allocate enough space as a dedicated device ahead of time.
...
You can configure your load balancer to use any method for distributing client requests, but in most contexts a round robin strategy is ideal. Make sure the load balancer is accessible through an IP address that is available to all machines on your network (including the Anypoint Platform PCE nodes)
...
* https://www.f5.com/products/nginx[Nginx]
