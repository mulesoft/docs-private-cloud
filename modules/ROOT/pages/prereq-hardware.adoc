...
To ensure the performance and stability of Anypoint Platform Private Cloud, every node in your Anypoint Platform Private Cloud environment must meet the requirements described in this topic. 

[IMPORTANT]
Before you install Anypoint Platform Private Cloud, your infrastructure team must review each of the following sections and verify that your environment meets the stated disk and device requirements. If needed, contact your MuleSoft representative for assistance.
...
[IMPORTANT]
If the stated requirements for Memory and CPU are not met, Anypoint Platform Private Cloud installation will fail. 
...
|HDD 2 xref:prereq-hardware#system-state-directory-device[System State Directory Device] | 100 GB |1500|/var/lib/gravity | Stores system configuration and metadata, including databases and packages. As package sizes can be large, it is important to estimate the minimum size requirements and allocate enough space as a dedicated device before installation.
...
|HDD 4 xref:prereq-hardware#docker-device[Docker Device] | 100 GB |3000|Not mounted | Used by Docker’s Device Mapper storage driver. A minimum of 100Gb is required for the Device Mapper directory. Devices with 50Gb or less will experience degraded system performance or might not work at all.
...
In some cases, when a Docker device is in use by another logical volume or you want to manually reset a Docker device previously configured for Device Mapper, the following Logical Volume Manager commands can be useful.
...
Anypoint Platform Private Cloud must run in a production environment with multiple servers. To distribute traffic among servers and to restrict access to specific ports, you must provide and configure a load balancer.

You can use any load balancer, including NGINX, but NGINX is not required.

[NOTE]
If using the xref:prereq-aws-terraform.adoc[AWS Provisioner for Anypoint Private Cloud], skip this section. An Amazon ELB is created during installation with the required configuration.
...
[NOTE]
Do not configure SSL certificates in your load balancer. TLS termination is handled by the platform with the certificates configured using Access Management. See xref:config-workflow.adoc[Post-installation Tasks].
