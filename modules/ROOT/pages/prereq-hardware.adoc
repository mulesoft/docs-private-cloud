...
[NOTE]
Make sure this device is mounted AFTER mounting the System State device under `/var/lib/gravity/planet`. If you are usi system.d units to mount the volumes you can declare mount dependencies using `After=<device-name>.<mount>`
