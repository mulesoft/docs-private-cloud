...
Two cluster topologies are supported for Anypoint Platform Private Cloud Edition: 3-nodes & 6-nodes. See xref:supported-cluster-config.adoc[Supported Network Topologies]. The requirements for all nodes are the same.
...
* Application data: |tores application configuration and data. The amount of space required should be at leasat 250GB, but might vary depending on your specific use case. It is important to estimate the minimum size requirements and allocate enough space as a dedicated device ahead of time.
