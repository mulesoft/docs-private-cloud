...
You can also mount the etcd device using `/etc/fstab` or another method. Consult with your system administrator to 	determine the best way to meet this requirement. If you use the following example as a guide for this process, specify the correct device name in two places.	
...
Unless specified, Docker configuration defaults to the use of the overlay driver, which is not recommended for production.
